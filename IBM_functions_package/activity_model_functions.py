...
def generate_step_stimulus(tend, ston, stoff, dt, stamp=1):
    """
    Generate an olfactory step stimulus.
...
def adaptation_dynamics(adaptval, CaKCval, tauadapt, adaptscale, dt):
    """
    Simulate the adaptation dynamics for each stimulus time step for a given KC neuron.
...
def inhibition_dynamics(inhval, CaKCval, tauinh, inhscale, dt):
    ...
    CaKCval : float
        ...
        Possibly I will be using weighted average here to get the inhibition driven by other KCs. 
        Weights will be KC-KC synaptic connectivity strength
        For the example case in this script autoinhibition is used.
    tainh : float
    ...
    dt : float
        Time step size (i.e. inverse of sampling rate) of the stimulation in seconds.
    cnnmat : n-D array, optional
        KC-KC synaptic connectivity matrix with the shape nKC*nKC. The default is None.
...
def activity_dependent_inhibition_modulation_sigmoidal(CaKCval, modinf, modslf):
    ...
    Parameters
    ----------
    CaKCval : float or 1D array
        The activity degree of the given cell(s).
    modinf : float
...
def KC_Ca_rate_dynamics(CaKCval, stval, tauKCdec, tauinp, inpscale, dt, n_str=0, nval=0, bline=0):
    """
    Simulate the KC activity dynamics (i.e. comparable to calcium levels) for each stimulus time step for single neuron.
    NOTE THAT this function does not have either adaptation or inhibition dynamics encorporated, to model adaptation and inhibition
    the respective functions should be coupled with this function in the simulation loop. 
    I think this is the cleanest way to parallelize all processes (for each iteration time step all variables for each KC are calculated in parallel,
    then lateral inhibition is calculated by pooling over all KCs and next iteration starts. Maybe this is extremely dumb and time will show.)
    In its own, this function supports also vectorized model (i.e. multiple KCs Ca traces packed into a t*n matrix)

    Different rise and decay time constants: https://www.researchgate.net/publication/253329470_Asymmetric_Time_Constants
    ...
    CaKCval : float
        Current value of the KC calcium trace (i.e. KC activation degree).
    stval : float
        Current value of the olfactory stimulation paradigm (comparable to odor concentration).
    tauKCdec : float
        Time constant for calcium decay in KC.
    tauinp : float
        Time constant for input-driven activity dynamics in KC.
    inpscale : float or 1-D array
        Degree of input activation in KC(s if population simulation is run).
    dt : float
        Time step size (i.e. inverse of sampling rate) of the stimulation in seconds.
    n_str : float or 1-D array
        Noise strength of the KC population. This is the standard deviation of the OU process in steady state for t towards infinity.
        Therefore it requires normalization by sqrt(2) to get the standard deviation of the OU process instead of the Wiener process.
        See page for on https://www.stat.uchicago.edu/~lalley/Courses/391/Lecture12.pdf for more details.
        Better is https://en.wikipedia.org/wiki/Ornstein%E2%80%93Uhlenbeck_process Numerical simulations part xD
    nval : float or 1-D array
        Current value of the noise process in the KC population. Standardly this is ~N(0, n_str**2)
    bline : float or 1-D array
        Baseline value of the KC calcium trace
    Returns
    -------
    newCaKC
        Updated value of the calcium trace in the next step.

    """
    n_str *= np.sqrt(2 * dt * (1/tauKCdec + stval*inpscale/tauinp))
    return CaKCval + (inpscale*stval - CaKCval + bline) * (1/tauKCdec+stval*inpscale/tauinp) * dt + (n_str * nval)
    # return CaKCval + (inpscale*stval/tauinp - CaKCval/tauKCdec) * dt THIS IS VERY WRONG xDDDDDDD


def DAN_cAMP_dynamics(danval, taudan, dt, shock=0, valpred=0, learnrate=1):
    """
    Model the cAMP decay following a DAN spiking. 
    DAN spiking is done elsewhere in the loop, that it increases by 1 for each spike.

    Parameters
    ----------
    danval : float
        Current value of the DAN activity driven cAMP levels.
    taudan : float
        DAN time constant.
    dt : float
        Time step size (i.e. inverse of sampling rate) of the stimulation in seconds.
    ...
    valpred : float, optional
        Current predicted valence for the given odor.
        Specifically, it is calculated as (MBON_act_untrained - MBON_act_current) / MBON_act_untrained. This way it is 1 if the given MBON is completely silenced,
        and 1 if the MBON activity happens without any synaptic weight change.
        The default is 0.
    learnrate : float, optional
        Learning rate specifying the synaptic weight change magnitude. The default is 1. This scales the prediction error signal.
        This will possibly be decepreated since learning rate should NOT scale the prediction error signal. It will instead be used in the weight change function.
    Returns
    -------
    new_cAMP : float
        Updated value of DAN activity driven cAMP levels.
    ...
    """
    Prediction error ideas
    ----------------------
    #       One way is to multiply/scale the shock stimulus with the current MBON activity. This is the prediction error signal.
    #       Another possibility is to do it subtractive -> assume another MBON of opposite valence (i.e. avoidance) which shows no activity change.
    #       Then you can subtract the activity of this MBON from the activity of the MBON you are interested in. This is the prediction error signal.
    #       Note you need to normalize the MBON activity difference to the oppositve valence MBON activity so that when learning completely happens
    #       cAMP signal is not driven high anymore as a consequence of prediction error=0.
    """
    # Note that prediction error signal is shock - valpred. The prediction error is subtractive since it is in the paranthesis which is also subtractive.
    # Therefore the prediction error is actually additive. Multiplicative shock is there to turn on prediction error signal only when there is shock happening.
    # TODO: DOES THIS MAKE EVEN SENSE????
    # Learnrate mediates the magnitude of the dopamine signal, you might want to replace this into the synaptic weight change function.
    # Possibly this mathematically does not really matter, since either you scale the DA levels then do the coincidence-based weight change,
    # or you take the unscaled DA levels, multiply with KC Ca levels and then scale down. Since DAN activity is linear, these two should be equivalent.
    # THIS HOLDS ONLY AND ONLY IF DAN ACITIVTY IS A LINEAR DIFFERENTIAL EQUATION
    # ACTUALLY THIS CHANGES THE VALENCE OF THE US, SO YOU MUST TRANSFER THE LEARNING RATE TO WEIGHT CHANGE FUNCTION. DONE!
    shock_on = shock > 0 # turn on the valence prediction circuitry only when there is shock happening
    new_cAMP = danval - (danval - shock_on*(shock - valpred) * learnrate) / taudan*dt
    return np.clip(new_cAMP, 0, None) # Make sure cAMP levels are always non-negative
...
#Example model -> Works as in testing script 
