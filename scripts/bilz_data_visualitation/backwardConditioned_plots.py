...
backwardConditioned_flys = load_trial(backwardConditioned)

# Dictionary to hold all flies' data
backwardConditioned_flys_data = {}

for fname, (pre_df, post_df) in backwardConditioned_flys.items():
    ...
    backwardConditioned_flys_data[fly_name] = {"pre": blocks_pre, "post": blocks_post}

# Compute global colormap bounds from PRE condition only
all_odor_data = [
    df_block
    for fly_data in backwardConditioned_flys_data.values()
    for df_block in fly_data["pre"].values()
...
print(f"DF/F extrems backward condtioned")
print(f"max = {global_max}, min = {global_min}")

# Create save directories
base_path = "/home/baxter/IBM/material/figures/bilz_visulaisation/backwardConditioned"
...
# Plot for each fly
for fly_name, recordings in backwardConditioned_flys_data.items():
    blocks_pre = recordings["pre"]
    blocks_post = recordings["post"]
