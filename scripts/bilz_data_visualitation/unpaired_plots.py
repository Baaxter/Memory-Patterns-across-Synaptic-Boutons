...
unpaired_flys = load_trial(unpaired)

# Dictionary to hold all flies' data
unpaired_flys_data = {}

for fname, (pre_df, post_df) in unpaired_flys.items():
    ...
    unpaired_flys_data[fly_name] = {"pre": blocks_pre, "post": blocks_post}

# Compute global colormap bounds from PRE condition only
all_odor_data = [
    df_block
    for fly_data in unpaired_flys_data.values()
...
print(f"DF/F extrems unpaired (control)")
print(f"max = {global_max}, min = {global_min}")

# Create save directories
base_path = "/home/baxter/IBM/material/figures/bilz_visulaisation/unpaired"
...
# Plot for each fly
for fly_name, recordings in unpaired_flys_data.items():
