...
for dataset in datasets:
    ...
    for fly_name, recordings in fly_data_dict.items():
        ...
        gs = gridspec.GridSpec(nrows=4, ncols=2, hspace=0.3, wspace=0.15)

        for row_idx, odor in enumerate(odor_labels):
            for col_idx, phase in enumerate(["pre", "post"]):
                ...
                ax.set_yticklabels(group_labels if col_idx == 0 else [])
                ...
                ax.set_title(f'{odor}-Pre' if phase == "pre" else f'{odor}-Post')

        # Shared colorbar
        cbar_ax = fig.add_axes([0.86, 0.11, 0.015, 0.85])
        cbar = fig.colorbar(im, cax=cbar_ax)
        cbar.set_label(r'$\Delta F/F_0$ [%]')
        cbar.ax.text(-3, -0.1, f'Min={vmin:.1f}, Max={vmax:.1f}',
                     transform=cbar.ax.transAxes,
                     ha='center', va='top',
                     fontsize='small')

        # Global time axis label
        fig.text(0.44, 0.04, 't [s]', ha='center')
